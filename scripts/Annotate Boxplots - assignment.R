{
  ...
  # Method 1
  # Create Relative Humdity variables based on whether precipitation or not
  noPrecHum = weatherData$relHum[weatherData$precipitation==0]
  PrecHum =weatherData$relHum[weatherData$precipitation==1]
  
  # Find stats (critical values)
  mean(noPrecHum)
  median(noPrecHum)
  sd(noPrecHum)
  mean(PrecHum)
  ...
  plot3 = ggplot()+
    # No precipitation
    annotate(geom="rect", 
             xmin = 1, ymin = q[2], # starting x and y coords
             xmax = 1.4, ymax = q[4], # ending x and y coords
    ...
    annotate(geom="segment",
             x=1,     y=mean(noPrecHum),      # line starts at (x,y)
             xend=1.4,  yend=mean(noPrecHum),   # line ends at (xend,yend)
             color = rgb(red=1, green=.4, blue=0)) +   # all red, some green makes orange
    
    # Upper
    annotate(geom="segment",
             x=1.2,     y=q[4],      # line starts at (x,y)
             xend=1.2,  yend=min(high, max(noPrecHum)),   # line ends at (xend,yend)
             color = rgb(red=1, green=.4, blue=0)) +   # all red, some green makes orange
    # Lower
    annotate(geom="segment",
             x=1.2,     y=q[2],      # line starts at (x,y)
             xend=1.2,  yend=max(low, min(noPrecHum)),   # line ends at (xend,yend)
             color = rgb(red=1, green=.4, blue=0)) +  # all red, some green makes orange
   
     # Precipitation
    annotate(geom="rect", 
             xmin = 1.6, ymin = q2[2], # starting x and y coords
             xmax = 2.0, ymax = q2[4], # ending x and y coords
             alpha = 0.5,    # transparency: 0=fully transparent, 1=fully opaque
             fill= "green",  # background color
             color= "blue",  # outline color
             linetype = 4) + # outline shape
    ...
    annotate(geom="segment",
             x=1.6,     y=mean(PrecHum),      # line starts at (x,y)
             xend=2.0,  yend=mean(PrecHum),   # line ends at (xend,yend)
             color = rgb(red=1, green=.4, blue=0)) +   # all red, some green makes orange
    
    # Upper
    annotate(geom="segment",
             x=1.8,     y=q2[4],      # line starts at (x,y)
             xend=1.8,  yend=min(high2, max(PrecHum)),   # line ends at (xend,yend)
             color = rgb(red=1, green=.4, blue=0)) +   # all red, some green makes orange
    # Lower
    annotate(geom="segment",
             x=1.8,     y=q2[2],      # line starts at (x,y)
             xend=1.8,  yend=max(low2, min(PrecHum)),   # line ends at (xend,yend)
             color = rgb(red=1, green=.4, blue=0)) +  # all red, some green makes orange
    
    # Need to use scale_x_continuous instead of scale_x_discrete when doing manual boxplots
    scale_x_continuous(breaks = c(1.2, 1.8),       # probably want to change these positions...
                       labels = c("No", "Yes")) +
  ...
  # Method 2 - For loop 
  ### For some reason, it didn't work through - haven't got a chance to debug ###
  
  plot4 = ggplot()+
    
    for(i in 1:2){
      dataInput = weatherData$relHum[weatherData$precipitation==(i-1)]
      m = mean(dataInput)
      max = max(dataInput)
      min = min(dataInput)
      q <- quantile(dataInput)
      box <- q[4]-q[2]
      high <- q[4]+1.5*box
      low <- q[2]-1.5*box
      
      annotate(geom="rect", 
               xmin = i, ymin = q[2], # starting x and y coords
               xmax = i+0.5, ymax = q[4], # ending x and y coords
               alpha = 0.5,    # transparency: 0=fully transparent, 1=fully opaque
               fill= "green",  # background color
               color= "blue",  # outline color
               linetype = 4) + # outline shape
      ...
      annotate(geom="segment",
                 x= i,     y=m,      # line starts at (x,y)
                 xend= i+0.5,  yend=m,   # line ends at (xend,yend)
                 color = rgb(red=1, green=.4, blue=0)) +   # all red, some green makes orange
        
      # Upper
      annotate(geom="segment",
                 x= i+0.25,     y=q[4],      # line starts at (x,y)
                 xend= i+0.25,  yend=min(high, max),   # line ends at (xend,yend)
                 color = rgb(red=1, green=.4, blue=0)) +   # all red, some green makes orange
      # Lower
      annotate(geom="segment",
                 x=i+0.25,     y=q[2],      # line starts at (x,y)
                 xend=i+0.25,  yend=max(low, min),   # line ends at (xend,yend)
                 color = rgb(red=1, green=.4, blue=0))  # all red, some green makes orange
    }

    # Need to use scale_x_continuous instead of scale_x_discrete when doing manual boxplots
    scale_x_continuous(breaks = c(1.25, 2.25),       # probably want to change these positions...
                       labels = c("No", "Yes")) +
